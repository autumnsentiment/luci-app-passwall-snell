...
- 自动创建、修复并选择插件专属 PassWall SOCKS 节点，不再要求手工修改 PassWall UCI 配置。
- 仅在 Bridge 已启用且活跃节点参数完整时接管流量，避免空配置导致 PassWall 断网。
- 禁用或卸载时恢复原 TCP/UDP 节点，只清理带插件标记的自管节点，不覆盖同名用户节点。
- 修复 OpenWrt SDK 构建未安装独立节点编辑页的问题。

## v1.0.3

- 将节点切换操作移到活动节点设置区，选择节点后可直接激活并重启代理链路。
- 节点表格恢复为摘要、排序和编辑用途，不再混入切换按钮。
